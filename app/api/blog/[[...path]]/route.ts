...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      console.warn("[Blog API Proxy] Error resolving params:", paramsError);
      resolvedParams = {};
    }

    console.log(
      "[Blog API Proxy] Resolved params:",
      JSON.stringify(resolvedParams)
    );

    const path = resolvedParams?.path;
    console.log(
      "[Blog API Proxy] Path from params:",
      path,
      "Type:",
      typeof path,
      "IsArray:",
    ...
    console.log("[Blog API Proxy] Path string:", pathString);
    ...
    // Build the API URL - handle empty path (for blog listing)
    const pathPart = pathString ? `/${pathString}` : "";
    const apiUrl = `${backendUrl}/api/blog${pathPart}${
      queryString ? `?${queryString}` : ""
    }`;

    // Debug logging
    console.log("[Blog API Proxy] Proxying request:", {
    ...
    try {
      ...
      console.error("[Blog API Proxy] JSON Parse Error:", {
        error:
    ...
    console.error("[Blog API Proxy] Unexpected error:", error);
    console.error("[Blog API Proxy] Error details:", {
